...
describe("createDemoSession seed", () => {
  it("seeds six present people with the host pinned first", () => {
    const snap = createDemoSession(clock).snapshot();
  ...
  it("snapshot is a copy — callers cannot mutate internal state", () => {
    const d = createDemoSession(clock);
...
describe("transitions", () => {
  ...
  it("stamps a manual add with the time it happens, not the demo-open time", () => {
    let t = NOW;
    const d = createDemoSession(() => t);
    t = NOW + 5 * 60 * 1000; // five minutes into the demo
    expect(d.add("Zoe Tan").participants.at(-1).joinTime).toBe(t);
  });
...
describe("randomize", () => {
  it("keeps introduced people in their slots and the host pinned", () => {
    const d = createDemoSession(clock, noShuffle);
