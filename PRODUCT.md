...
- **The host** runs `uv run tracker.py`, opens the page, and screen-shares it. They are the only person who clicks anything: toggling "introduced" as each person speaks, occasionally adding a name manually, hitting reset at the end. They are scanning at desktop distance under meeting-attention split.
...
It runs as one local process. No Zoom credentials, no SaaS, no account, no history. State lives for the meeting and resets when the host stops the process. That ephemerality is deliberate: this is a per-meeting utility, not a CRM.
...
4. **Per-meeting ephemerality is a feature.** State is in memory and resets on process exit. The UI must not imply otherwise: no history affordances, no "previous meetings" links, no analytics, no exports. Anything that hints at persistence is a lie.
